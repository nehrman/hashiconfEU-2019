resource "kubernetes_deployment" "fruits-catalog" {
  depends_on = ["vault_kubernetes_auth_backend_role.fruits-catalog","vault_database_secret_backend_role.mongodb"]
      ...
      spec {
        ...
        container {
          ...
          command = [
            "/bin/sh",
            "-c",
            "X_VAULT_TOKEN=$(cat /etc/vault/token);VAULT_LEASE_ID=$(cat /etc/app/creds.json | jq -j '.lease_id');while true; do curl -k -s --request PUT --header \"X-Vault-Token: $X_VAULT_TOKEN\" --data '{\"lease_id\": \"'\"$VAULT_LEASE_ID\"'\", \"increment\": 3600}' http://192.168.94.141:8200/v1/sys/leases/renew;echo 'Waiting for 360 sec before renewing lease';sleep 360;done",
          ]
            ...
            pre_stop {
              exec {
                command = [
                  "/bin/sh",
                  "-c",
                  "X_VAULT_TOKEN=$(cat /etc/vault/token);VAULT_LEASE_ID=$(cat /etc/app/creds.json | jq -j '.lease_id');curl -k --request PUT --header \"X-Vault-Token: $X_VAULT_TOKEN\" --data '{\"lease_id\": \"'\"$VAULT_LEASE_ID\"'\"}' http://192.168.94.141:8200/v1/sys/leases/revoke;",
                ]
              }
            }
          }
